...
func (tester *UnStakeTester) noStakeRecord(t *testing.T, d *Dandelion) {
	...
	pub3, _ := priv3.PubKey()
	a.NoError(tester.acc0.SendTrx(AccountCreate(name0, acct3.Name, pub3,constants.DefaultAccountCreateFee, "")))
	a.NoError(d.ProduceBlocks(1))
	acct4 := d.Account(name4)
	priv4, _ := prototype.GenerateNewKey()
	pub4, _ := priv4.PubKey()
	a.NoError(tester.acc0.SendTrx(AccountCreate(name0, acct4.Name, pub4, constants.DefaultAccountCreateFee, "")))
	a.NoError(d.ProduceBlocks(1))
